...
const stopDefs: Array<{
  ...
  {
    index: 2, name: 'Japan', country: 'Japan',
    coords: [137.5, 36.0],
  ...
  {
    index: 3, name: 'London', country: 'United Kingdom',
    coords: [-0.12, 51.51],
  ...
  {
    index: 4, name: 'Regina, SK', country: 'Canada',
    coords: [-104.62, 50.45],
  ...
  {
    index: 5, name: 'Waterloo', country: 'Canada',
    coords: [-80.52, 43.47],
    period: 'Now · CS 1B @ UW',
...
export default function JourneyMap() {
  // Pre-compute country paths on the server
  const countryPaths = features
    .map((f: any) => pathGen(f))
    .filter(Boolean) as string[]

  // Pre-compute arc paths on the server
  const arcPaths = stopDefs.slice(0, -1).map((s, i) =>
    curvePath(s.coords, stopDefs[i + 1].coords)
  )

  // Pre-compute projected stop positions on the server
